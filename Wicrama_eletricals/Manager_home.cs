...
namespace WicramaElectricals
{
    public partial class Manager_home : Form
    {
        ...
        private void Manager_home_Load(object sender, EventArgs e)
        {
            ...
            u_itemcodeTxt.Text = "ID";
            FillUpdateGridView();

            d_itemcodeTxt.Enabled = false;
            d_itemcodeTxt.Text = "ID";
            FilldeleteGridView();

            p_order_idTxt.Enabled = false;
            p_order_idTxt.Text = "ID";
            FillPaidGridView();
            /*
            unp_orderidTxt.Enabled = false;
            unp_orderidTxt.Text = "ID";
            FillUnpaidGridView();
            */
        ...
        {
            slide_panel.Width = update.Width;
            slide_panel.Left = update.Left;
        ...
        {
            slide_panel.Width = delete.Width;
            slide_panel.Left = delete.Left;
        ...
        {
            slide_panel.Width = paid_orders.Width;
            slide_panel.Left = paid_orders.Left;
        ...
        private void unpaid_orders_Click(object sender, EventArgs e)
        {
            ...
            slide_panel.Width = unpaid_orders.Width;
            slide_panel.Left = unpaid_orders.Left;
        ...
        {
            MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=wicrama;");
            MySqlDataAdapter sda = new MySqlDataAdapter("select * from items ", conn);
        ...
        private void itemlist_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            if(e.RowIndex >= 0)
            {
                ...
                I_itemcode.Text = row.Cells[0].Value.ToString();
                I_brand.Text = row.Cells[1].Value.ToString();
                I_itemname.Text = row.Cells[2].Value.ToString();
                comboBox1.Text = row.Cells[3].Value.ToString();
                I_price.Text = row.Cells[4].Value.ToString();
                I_instock.Text = row.Cells[5].Value.ToString();
            }
        }
        ...
        private void u_itemBtn_Click(object sender, EventArgs e)
        {
            if (u_modelTxt.Text != "" && u_partTxt.Text != "" && u_typeCombo.Text != "" && u_priceTxt.Text != "" && u_stockTxt.Text != "")
            {
                ...
                {
                    MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=wicrama;");
                    string query = "update `items` set `model`= '" + u_modelTxt.Text + "',`part`= '" + u_partTxt.Text + "',`type`= '" + u_typeCombo.Text + "',`price`='" + u_priceTxt.Text + "', `instock`= '" + u_stockTxt.Text + "'where `id`= '"+ u_itemcodeTxt.Text +"' ";
                    MySqlCommand cmd = new MySqlCommand(query, conn);
                    conn.Open();
                    cmd.ExecuteNonQuery();
                    conn.Close();
        ...
        {
            MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=wicrama;");
            MySqlDataAdapter u_sda = new MySqlDataAdapter("select * from items ", conn);
        ...
        {
            MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=wicrama;");
            MySqlDataAdapter d_sda = new MySqlDataAdapter("select * from items ", conn);
        ...
        void FillUnpaidGridView()
        {
            //String status = "no";
            MySqlConnection conn = new MySqlConnection(@"datasource=127.0.0.1;port=3306;SslMode=none;username=root;password=;database=wicrama;");
            MySqlDataAdapter unp_sda = new MySqlDataAdapter("select * from orders ", conn);
        ...
        //The function tht executes when the generate pdf button is clicked
        private void unp_cancelorder_btn_Click(object sender, EventArgs e)
        {
            exportgridtopdf(unp_dataGridView, "Order-List Report");
        }

        private void itemlist_export_Click(object sender, EventArgs e)
        {
