    ...
    public partial class Home : Form
    {
        ...
        private void bunifuFlatButton1_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MySqlDataAdapter sda = new MySqlDataAdapter("select * from items where model='" + comboBox1.Text + "' && part='" + comboBox2.Text + "'", conn);
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            try
            {
                if (int.Parse(textBox6.Text) >= int.Parse(textBox7.Text))
                {
                    if(arraycount<10)
                    {
                        ItemList[arraycount] = textBox2.Text + " " + textBox3.Text + " " + textBox4.Text + " " + textBox5.Text + "*" + textBox7.Text + Environment.NewLine;
                        TotalPrice += float.Parse(textBox5.Text) * float.Parse(textBox7.Text);
                        UpdateQuery += "update items set instock='" + (int.Parse(textBox6.Text) - int.Parse(textBox7.Text)) + "' where id='" + textBox1.Text + "';";
                        String msg = textBox1.Text + " " + textBox2.Text + " " + textBox3.Text + " " + textBox4.Text + "*" + textBox7.Text;
                        MessageBox.Show(msg + Environment.NewLine + "Added to List");
                        arraycount++;
                    }
                    else
                        MessageBox.Show("Maximum 10 Items");
                }
                else
                {
